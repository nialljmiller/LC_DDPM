...
def my_collate(batch):
    ...
    batch = [torch.tensor(tup[0]).type(torch.float) for tup in sorted_batch]
    labels = [tup[1] for tup in sorted_batch]

    labels = torch.tensor(labels)
    errs = torch.tensor(errs)

    batch = pack_sequence(batch, enforce_sorted=True)

    return batch, labels, errs
...
class Lightcurves(data.Dataset):
    def __init__(self, folder, lc_size, minmaxnorms=(0, 5.5)):
        super().__init__()
        self.folder = folder
        self.lc_size = lc_size
        self.paths = list(Path(f'{folder}').glob(f'**/*.csv'))
    ...
    def __getitem__(self, index):
        ...
        lc = np.loadtxt(path, delimiter=',', usecols=(0, 2), unpack = True)

        #lc = np.clip(lc, self.min_, self.max_)
        #lc = 2*(lc - self.min_)/(self.max_ - self.min_) - 1 # A min max norm for all maxima == 5 and minima == 0.0 gals
        #pad_sequence(torch.tensor(lc)).size()

        lc = lc.copy()
        return torch.tensor(lc)
...
class StochasticCurves(data.Dataset):
    def __init__(self, sequences, batch_size=512):
        ...
        print(batch_size)
    def __len__(self):
        # this needs to be a big number or we get errors
    ...
    def __getitem__(self, index):
        ...
        get_curve = True
        while get_curve:
            random_idx = index#np.random.randint(len(self.sequences))
            mag = self.sequences[random_idx][:, 0]
            magerr = self.sequences[random_idx][:, 1]
            phase = self.sequences[random_idx][:, 2]
            time = self.sequences[random_idx][:, 3]
    
            sequence = np.concatenate((mag, magerr, phase, time), axis=1)
            try:
                pass
            except:
                print(mag, phase, time)
                print('aaaaa')
                exit()
            sequence = sequence[np.argsort(phase)]
            get_curve = False
            print('pass')
        '''
...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        sequences_pickle = '/beegfs/car/njm/contrastive_curves/sequences_real.pkl'

        sequences = []
        tier = "/beegfs/car/njm/Periodic_Variables/Best/LC/*.csv"

        files = glob.glob(tier)
        for fi in tqdm(files[0:100]):
            mag, magerr, phase, time = np.genfromtxt(fi, dtype='float', converters = None, comments = '#', delimiter = ',').T
            if len(mag) > lc_size - 1:
                mag, magerr, phase, time = delete_rand_items(mag, magerr, time, phase,  len(mag)-lc_size)
                #np.tile(a, (3, 1))
                #sequence = np.stack((np.tile(mag,(2,2)), np.tile(magerr,(2,2)), np.tile(phase,(2,2)), np.tile(time,(2,2))), axis=0)
                sequence = np.stack((np.tile(mag,(2,2)), np.tile(magerr,(2,2)), np.tile(phase,(2,2))), axis=0)
                sequences.append(sequence)


        raw_sequences = sequences#pickle.load(sequences_pickle)

        sequences = [seq for seq in raw_sequences if ~np.any(np.isnan(seq))]
